class StaticClass {
	...
	for (int j = 0; j < img_Height-factor_escala; j++){
		for (int i = 0; i<img_Width-factor_escala; i++){
			
		Color Color_average;	
		Color_average = average_block (img, i, j, factor_escala);
		
		for (int y = 0; y < img_Height-factor_escala; y= y+factor_escala){
			for (int x = 0; x<img_Width-factor_escala; x = x+factor_escala){
				
				img.setColor(i,j,Color_average);
	...
	return img;
		...
		// for (int i=0; i< factor_escala; i++){
			// for (int j=0; j<factor_escala;j++){
				 
				// data [i][j] = img.getColor (i,j) ;
	 ...
	 */
		 ...
		 static ColorImage teste44 () {
			 
		ColorImage img = new ColorImage ("objc1.png");
		
		int factor_escala = 10;
